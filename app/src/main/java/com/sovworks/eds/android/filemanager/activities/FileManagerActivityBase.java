...
public abstract class FileManagerActivityBase extends RxAppCompatActivity implements PreviewFragment.Host {
    ...
    public void goTo(Location location) {
        goTo(location, 0, 0, false);
    }

    public void goTo(Location location, int scrollPosition, int scrollOffset, boolean clearHistory) {
        Logger.debug(TAG + ": goTo");
        closeIntegratedViewer();
        // 清空导航堆栈
        if (clearHistory) {
            final FileListDataFragment f = getFileListDataFragment();
            if (f != null) {
                f.getNavigHistory().clear();
            }
        }
        FileListViewFragment f = getFileListViewFragment();
        if (f != null) {
            // f.goTo(location, scrollPosition, scrollOffset, true);
            // clearHistory 为 True 时为点击左侧菜单进入, 因此不将其加入历史堆栈
            f.goTo(location, scrollPosition, scrollOffset, !clearHistory);
        }
    }
