...
Java_com_sovworks_eds_fs_exfat_ExFat_openFS(JNIEnv *env, jobject instance, jboolean readOnly) {
    int rc;
    struct exfat *ef;
...
Java_com_sovworks_eds_fs_exfat_ExFat_getAttr(JNIEnv *env, jobject instance, jobject fileStat,
                                             jstring pathString) {
    const char *path;
...
JNIEXPORT jint JNICALL
Java_com_sovworks_eds_fs_exfat_ExFat_updateTime(JNIEnv *env, jobject instance, jstring pathString,
                                                jlong time) {
...
JNIEXPORT jint JNICALL Java_com_sovworks_eds_fs_exfat_ExFat_read
        (JNIEnv *env, jobject instance, jlong handle, jbyteArray buf, jint bufOffset, jint count,
         jlong position) {
...
JNIEXPORT jint JNICALL
Java_com_sovworks_eds_fs_exfat_ExFat_makeFS(JNIEnv *env, jclass type, jobject raio,
                                            jstring labelString,
    ...
    int res = format_exfat(dev, label, (uint32_t) volumeSerial, (uint64_t) firstSector,
                           sectorsPerCluster);
    if (labelString)
